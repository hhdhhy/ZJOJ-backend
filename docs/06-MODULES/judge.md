...
2. 创建 Submission 记录 (status=PENDING)
        ↓
3. HydroJudgeClient.judge(submission)
        ↓
4. 读取测试用例 ZIP 文件
        ↓
5. 解压到临时目录
...
### Q1: go-judge 返回 "Time Limit Exceeded" 但 exitStatus=0

**原因**: cgroup v1 内存限制问题  
**解决**: adapter.py 中自行判断时间和内存，不依赖 go-judge 的 status 字段
