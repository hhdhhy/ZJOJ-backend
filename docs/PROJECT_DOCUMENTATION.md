...
| **MySQL** | 5.7+ | 关系型数据库 |
| **Celery** | Latest | 异步任务队列（可选） |
| **go-judge** | v1.11.4 | 代码执行沙箱 |
...
│   │
│   ├── judge/                     # ⚡ 代码评测模块
│   │   ├── adapter.py            # Python评测适配器
│   │   ├── hydro_client.py       # Django客户端
...
**技术架构**:
```
用户提交 → Submission创建 → HydroJudgeClient
         → JudgeAdapter → go-judge (/run API)
         → 比较输出 → 结果回写数据库
```

**核心组件**:
- `JudgeAdapter` - Python评测适配器，封装go-judge调用
- `HydroJudgeClient` - Django集成客户端
- `go-judge` - 底层沙箱服务 (localhost:5050)
...
Submission:
  ...
  - problem (ForeignKey)
  - code, language
  - status (PENDING/JUDGING/ACCEPTED/WA/TLE/MLE/RE/CE/SE)
...
| ⚡ **评测系统** | 代码评测系统集成说明 | [JUDGE_SYSTEM.md](JUDGE_SYSTEM.md) |
